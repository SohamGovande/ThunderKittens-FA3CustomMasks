...
@triton.jit
def _attn_fwd_inner(acc, l_i, m_i, q, bs_ptr, stride_bn, bm_ptr, stride_bmn, vn_ptr, stride_vnm, attn_mask_ptr, attn_mask_m,  #
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        # vn = tl.load(vn_ptr + bn * stride_vnm)
        qk = tl.dot(q, k)
        # Blocksparse - Sum and store the block's logits
        # tl.store(bs_ptr + bn * stride_bn, tl.sum(qk, 1))
        if STAGE == 2:
        ...
        # wp = p * vn[None, :]
        l_ij = tl.sum(p, 1)
        # Value weighted attention probs
        # tl.store(bs_ptr + bn * stride_bn, tl.sum(wp, 1))
        # Unweighted attention probs
        # tl.store(bs_ptr + bn * stride_bn, l_ij)
        tl.store(bm_ptr + bn * stride_bmn, m_ij)
        # -- update m_i and l_i
        ...
        # mask = (start_m * BLOCK_M + tl.arange(0, BLOCK_M)[:, None]) < n_unpadded
        # Unsqueeze along key dim
        mask = attn_mask_m[:, None]
        # wp = p * tl.where(mask, 1, 0) * tl.sum(v * v, 1)[None, :]
        wp = p * tl.where(mask, 1, 0)
        tl.store(bs_ptr + bn * stride_bn, tl.sum(wp, 1))
        if fp8_v:
            p = p.to(tl.float8e5)
        else:
...
@triton.jit
def _attn_fwd(Q, K, V, sm_scale, M, L, Out, BS, bm, vn, attn_mask,  #
              stride_qz, stride_qh, stride_qm, stride_qk,  #
              stride_kz, stride_kh, stride_kn, stride_kk,  #
              stride_vz, stride_vh, stride_vk, stride_vn,  #
              stride_oz, stride_oh, stride_om, stride_on,  #
              stride_bz, stride_bh, stride_bm, stride_bn,  # [b, h, n, n // 32]
              stride_bmz, stride_bmh, stride_bmm, stride_bmn,  # [b, h, n, n // 32]
              stride_vnz, stride_vnh, stride_vnm,  # [b, h, n]
    ...
    bs_ptr = BS + off_z.to(tl.int64) * stride_bz + off_h.to(tl.int64) * stride_bh + (start_m.to(tl.int64) * BLOCK_M + tl.arange(0, BLOCK_M)) * stride_bm
    bm_ptr = bm + off_z.to(tl.int64) * stride_bmz + off_h.to(tl.int64) * stride_bmh + (start_m.to(tl.int64) * BLOCK_M + tl.arange(0, BLOCK_M)) * stride_bmm
    vn_ptr = vn + off_z.to(tl.int64) * stride_vnz + off_h.to(tl.int64) * stride_vnh + tl.arange(0, BLOCK_N) * stride_vnm
    ...
    if STAGE & 1:
        acc, l_i, m_i = _attn_fwd_inner(acc, l_i, m_i, q, bs_ptr, stride_bn, bm_ptr, stride_bmn, vn_ptr, stride_vnm, attn_mask, attn_mask_m, K_block_ptr, V_block_ptr,  #
                                        start_m, qk_scale,  #
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale, attn_mask, return_ml=False):
        ...
        # [b, h, n]
        vn = torch.norm(v.float(), dim=-1)

        # [b, h, n, n_blocks]
        bs = torch.zeros(q.shape[0], q.shape[1], q.shape[2], n_blocks, device=q.device, dtype=torch.float32)
        # [b, h, n, n_blocks]
        bm = torch.zeros(q.shape[0], q.shape[1], q.shape[2], n_blocks, device=q.device, dtype=torch.float32)
        ...
        _attn_fwd[grid](
            q, k, v, sm_scale, M, L, o, bs, bm, vn, attn_mask,  #
            ...
            vn.stride(0), vn.stride(1), vn.stride(2),  #
            q.shape[0], q.shape[1],  #
            N_CTX=q.shape[2],  #
            HEAD_DIM=HEAD_DIM_K,  #
            STAGE=stage,  #
        ...
        # 1. Scale exp logit blocksums by final softmax scale
        diff = torch.exp2(bm - (M - torch.log2(L)).unsqueeze(-1))
        bs = bs * diff / L.unsqueeze(-1)

        # 2. Rearrange and sum m_blocks
        # print(f'pre bs sum: {bs.sum(dim=-1)}')
        bs = rearrange(bs, "b h (m g) s -> b h m g s", m=m_blocks, g=G_BM)
        bs = bs.sum(dim=-2)
        ...
        ctx.causal = causal
        if not return_ml:
            return o, bs.to(q.dtype)
        else:
            return o, bs.to(q.dtype), M, L
...
def vw_bs_triton_attention(q, k, v, causal, sm_scale, attn_mask, return_ml=False):
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, dtype=DTYPE):
    ...
    if pad and N_CTX % 64 != 0:
        ...
        tri_out, tri_bs = vw_bs_triton_attention(qp, kp, vp, causal, sm_scale, attn_mask)
        tri_out = tri_out[:, :, :N_CTX, :]
        print(f'tri_bs shape: {tri_bs.shape}')
    else:
        attn_mask = torch.ones((q.shape[2]), dtype=torch.bool, device="cuda")
        tri_out, tri_bs = vw_bs_triton_attention(q, k, v, causal, sm_scale, attn_mask)
    ...
    if N_CTX < 2048:
        # vn = torch.norm(v.float(), dim=-1)
        vn = torch.sum(v.float() * v.float(), -1)
        # reference implementation
        M = torch.tril(torch.ones((N_CTX, N_CTX), device="cuda"))
        p = torch.matmul(q, k.transpose(2, 3))

        p *= sm_scale
        p *= 1.44269504  # 1/log(2)
        m = torch.max(p, dim=-1, keepdim=True)[0]
        p = torch.exp2(p - m)
        ...
        if N_CTX % 64 != 0:
            padding = 64 - N_CTX % 64
            vp = torch.cat([vp, torch.zeros((b, h, padding, n), dtype=dtype, device="cuda")], dim=2)
            vp = torch.cat([vp, torch.zeros((b, h, n + padding, padding), dtype=dtype, device="cuda")], dim=3)
            print(f'vp shape: {vp.shape}')
        vp_blocks = vp.view(b, h, m_blocks, G_BM, n_blocks, G_BN)
        ref_bs = vp_blocks.sum(dim=(3, 5)).to(dtype)
        # p = torch.exp(p)
        ...
        print(f"tri_bs: {tri_bs[0, 0, 0].sort(descending=True).indices}")
        print(f"ref_bs: {ref_bs[0, 0, 0].sort(descending=True).indices}")
        print(f"tri_bs: {tri_bs[0, 0, -1]}")
        print(f"ref_bs: {ref_bs[0, 0, -1]}")
        # raise Exception("Stop here")
        torch.set_printoptions(profile="default")
        print(f'bs dist: {torch.dist(tri_bs, ref_bs, p=2)}')
        raise Exception("stop")
        assert torch.allclose(tri_bs, ref_bs, atol=1, rtol=0)
...
def bench_flash_attention(BATCH, H, N_CTX, HEAD_DIM, causal, mode, provider, device="cuda"):
    ...
    if "triton" in provider:
        ...
        attn_mask = torch.ones((q.shape[2]), dtype=torch.bool, device="cuda")
        fn = lambda: vw_bs_triton_attention(q, k, v, causal, sm_scale, attn_mask)
        if mode == "bwd":
            o = fn()
...
if __name__ == "__main__":
    ...
    # test_op(1, 3, 1024, 128, False)
    # test_op(1, 3, 1048, 128, False)
